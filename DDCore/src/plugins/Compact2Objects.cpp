...
namespace dd4hep
{
  ...
  template <>
  void Converter<World>::operator()(xml_h element) const;
  template <>
  void Converter<Plugin>::operator()(xml_h element) const;
  template <>
  void Converter<Constant>::operator()(xml_h element) const;
  template <>
  void Converter<Material>::operator()(xml_h element) const;
  template <>
  void Converter<Atom>::operator()(xml_h element) const;
  template <>
  void Converter<Isotope>::operator()(xml_h element) const;
  template <>
  void Converter<VisAttr>::operator()(xml_h element) const;
  template <>
  void Converter<Region>::operator()(xml_h element) const;
  template <>
  void Converter<Readout>::operator()(xml_h element) const;
  template <>
  void Converter<Segmentation>::operator()(xml_h element) const;
  template <>
  void Converter<LimitSet>::operator()(xml_h element) const;
  template <>
  void Converter<Property>::operator()(xml_h element) const;
  template <>
  void Converter<CartesianField>::operator()(xml_h element) const;
  template <>
  void Converter<SensitiveDetector>::operator()(xml_h element) const;
#if ROOT_VERSION_CODE >= ROOT_VERSION(6, 17, 0)
  template <>
  void Converter<OpticalSurface>::operator()(xml_h element) const;
  template <>
  void Converter<PropertyTable>::operator()(xml_h element) const;
  template <>
  void Converter<PropertyConstant>::operator()(xml_h element) const;
#endif
  template <>
  void Converter<DetElement>::operator()(xml_h element) const;
  template <>
  void Converter<STD_Conditions>::operator()(xml_h element) const;
  template <>
  void Converter<IncludeFile>::operator()(xml_h element) const;
  template <>
  void Converter<JsonFile>::operator()(xml_h element) const;
  template <>
  void Converter<XMLFile>::operator()(xml_h element) const;
  template <>
  void Converter<Header>::operator()(xml_h element) const;
  template <>
  void Converter<DetElementInclude>::operator()(xml_h element) const;
  template <>
  void Converter<Parallelworld_Volume>::operator()(xml_h element) const;
  template <>
  void Converter<Compact>::operator()(xml_h element) const;
}
...
static Ref_t create_ToroidField(Detector &description, xml_h e)
{
  xml_comp_t c(e);
  bool has_inner_radius = c.hasAttr(_U(inner_radius));
  bool has_outer_radius = c.hasAttr(_U(outer_radius));
  ...
  if (!has_inner_radius && !has_outer_radius)
  {
    throw_print("Compact2Objects[ERROR]: For a toroidal field at least one of the "
                " xml attributes inner_radius of outer_radius MUST be set.");
  }
  CartesianField obj;
  ToroidField *ptr = new ToroidField();
  //
  // This logic is a bit weird, but has its origin in the compact syntax:
  // If no "inner_radius" is given, the "outer_radius" IS the "inner_radius"
  // and the "outer_radius" is given by one side of the world volume's box
...
DECLARE_XMLELEMENT(ToroidMagnet, create_ToroidField)
// This is the plugin required for slic: note the different name
DECLARE_XMLELEMENT(toroid, create_ToroidField)
...
void Converter<Debug>::operator()(xml_h e) const
{
  for (xml_coll_t coll(e, _U(type)); coll; ++coll)
  {
    ...
    if (nam.substr(0, 6) == "isotop")
      s_debug.isotopes = (0 != val);
    else if (nam.substr(0, 6) == "elemen")
      s_debug.elements = (0 != val);
    else if (nam.substr(0, 6) == "materi")
      s_debug.materials = (0 != val);
    else if (nam.substr(0, 6) == "visatt")
      s_debug.visattr = (0 != val);
    else if (nam.substr(0, 6) == "region")
      s_debug.regions = (0 != val);
    else if (nam.substr(0, 6) == "readou")
      s_debug.readout = (0 != val);
    else if (nam.substr(0, 6) == "limits")
      s_debug.limits = (0 != val);
    else if (nam.substr(0, 6) == "segmen")
      s_debug.segmentation = (0 != val);
    else if (nam.substr(0, 6) == "consta")
      s_debug.constants = (0 != val);
    else if (nam.substr(0, 6) == "define")
      s_debug.constants = (0 != val);
    else if (nam.substr(0, 6) == "includ")
      s_debug.includes = (0 != val);
    else if (nam.substr(0, 6) == "matrix")
      s_debug.matrix = (0 != val);
    else if (nam.substr(0, 6) == "surfac")
      s_debug.surface = (0 != val);
    else if (nam.substr(0, 6) == "incgua")
      s_debug.include_guard = (0 != val);
  }
}
 ...
 */
template <>
void Converter<Constant>::operator()(xml_h e) const
{
  if (e.tag() != "include")
  {
 ...
 */
template <>
void Converter<Header>::operator()(xml_h e) const
{
  xml_comp_t c(e);
 ...
 */
template <>
void Converter<Material>::operator()(xml_h e) const
{
  xml_ref_t x_mat(e);
  ...
  if (0 == mat)
  {
    ...
    if (!density.ptr())
    {
      throw_print("Compact2Objects[ERROR]: material without density tag ( <D  unit=\"g/cm3\" value=\"..\"/> ) provided: " + string(matname));
    }
 ...
 */
template <>
void Converter<Isotope>::operator()(xml_h e) const
{
  xml_dim_t isotope(e);
  TGeoManager &mgr = description.manager();
 ...
 */
template <>
void Converter<Atom>::operator()(xml_h e) const
{
  xml_ref_t elem(e);
 ...
 */
template <>
void Converter<STD_Conditions>::operator()(xml_h e) const
{
  xml_dim_t cond(e);
  // Create the isotope object in the event it is not yet present from the XML data
  if (cond.ptr())
...
void Converter<OpticalSurface>::operator()(xml_h element) const
{
  ...
  Double_t value = 1.0;
  if (e.hasAttr(_U(type)))
    type = OpticalSurface::stringToType(e.attr<string>(_U(type)));
  if (e.hasAttr(_U(model)))
    model = OpticalSurface::stringToModel(e.attr<string>(_U(model)));
  if (e.hasAttr(_U(finish)))
    finish = OpticalSurface::stringToFinish(e.attr<string>(_U(finish)));
  if (e.hasAttr(_U(value)))
    value = e.attr<double>(_U(value));
 ...
 */
template <>
void Converter<PropertyConstant>::operator()(xml_h e) const
{
 ...
 */
template <>
void Converter<PropertyTable>::operator()(xml_h e) const
{
  vector<double> vals;
 ...
 */
template <>
void Converter<VisAttr>::operator()(xml_h e) const
{
 ...
 */
template <>
void Converter<Region>::operator()(xml_h elt) const
{
  xml_dim_t e = elt;
 ...
 */
template <>
void Converter<Segmentation>::operator()(xml_h seg) const
{
 ...
 */
template <>
void Converter<Readout>::operator()(xml_h e) const
{
  ...
  {
    string hits_key;
    if (colls.hasAttr(_U(key)))
      hits_key = colls.attr<string>(_U(key));
 ...
 */
template <>
void Converter<LimitSet>::operator()(xml_h e) const
{
  Limit limit;
 ...
 */
template <>
void Converter<Property>::operator()(xml_h e) const
{
  string name = e.attr<string>(_U(name));
 ...
 */
template <>
void Converter<CartesianField>::operator()(xml_h e) const
{
  string msg = "updated";
  string name = e.attr<string>(_U(name));
...
void Converter<SensitiveDetector>::operator()(xml_h element) const
{
  ...
  try
  {
    ...
    { // If no unit is given , we assume the correct Geant4 unit is used!
...
void Converter<DetElement>::operator()(xml_h element) const
{
  ...
  try
  {
    ...
    if (!par_name.empty())
    {
      ...
      if (par_name[0] == '$')
        par_name = xml::getEnviron(par_name);
...
void Converter<World>::operator()(xml_h element) const
{
  ...
  Material mat = world.hasAttr(_U(material))
                     ? description.material(world.attr<string>(_U(material)))
                     : description.air();

  if (!vis.isValid())
    vis = description.invisible();
